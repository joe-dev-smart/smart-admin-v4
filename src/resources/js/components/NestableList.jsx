...
export default function NestableList() {
  ...
  const handleDragEnd = event => {
    ...
    const reordered = arrayMove(items, oldIndex, newIndex).map(item => item.id === active.id ? {
      ...item,
      depth: shadowDepth
    } : item);
    setItems(reordered);
  ...
  return <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragMove={handleDragMove} onDragEnd={handleDragEnd}>
      <SortableContext items={items.map(i => i.id)} strategy={verticalListSortingStrategy}>
        {items.map(item => <Item key={item.id} id={item.id} depth={item.id === activeId ? shadowDepth : item.depth} />)}
      </SortableContext>
