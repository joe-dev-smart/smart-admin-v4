...
const UserProfile = () => {
  return <div className="sidenav-user">
      <div className="d-flex justify-content-between align-items-center">
        ...
        <Dropdown>
          <DropdownToggle as={'a'} role="button" aria-label="profile dropdown" className="dropdown-toggle drop-arrow-none link-reset sidenav-user-set-icon">
          ...
          <DropdownMenu>
            {userDropdownItems.map((item, idx) => <Fragment key={idx}>
                {item.isHeader ? <div className="dropdown-header noti-title">
                  ...
                  </div> : item.isDivider ? <DropdownDivider /> : <DropdownItem as={Link} to={item.url ?? ''} className={item.class}>
