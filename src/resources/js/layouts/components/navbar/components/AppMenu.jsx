...
const MenuItemWithChildren = ({
  ...
  return <Dropdown as={menuLevel > 1 ? 'div' : 'li'} drop={menuLevel > 1 ? 'end' : 'down'} className={`${wrapperClass ?? ''} ${isActive ? 'active' : ''}`}>
...
const MenuItem = ({
  ...
  const link = <Link href={item.url ?? '/'} className={`${linkClass ?? ''} ${isActive ? 'active' : ''}`}>
      {item.icon && <span className="menu-icon">
          <item.icon />
        </span>}
      <span className="menu-text">{item.label}</span>
...
const AppMenu = () => {
  return <div className="collapse navbar-collapse">
      <ul className="navbar-nav">
        {horizontalMenuItems.map((item, idx) => <Fragment key={idx}>
