...
import { Button, Col, Dropdown, DropdownItem, DropdownMenu, DropdownToggle, Image, Row } from 'react-bootstrap';
      ...
      <Dropdown align="end">
        ...
        <DropdownMenu className="p-0 dropdown-menu-end dropdown-menu-lg">
          <div className="px-3 py-2 border-bottom">
            <Row className="align-items-center">
              ...
              <Col className="text-end">
                <Link href="#" className="badge badge-soft-success badge-label py-1">
                  09 Notifications
                </Link>
              </Col>
            </Row>
          ...
          maxHeight: '300px'
            ...
            {messages.map(message => <DropdownItem className={`notification-item py-2 text-wrap ${message.active ? 'active' : ''}`} id={message.id} key={message.id}>
                <span className="d-flex gap-3">
                  {message.user.icon && <span className="avatar-md flex-shrink-0">
                      <span className={`avatar-title rounded-circle fs-22 ${message.user.bgClass}`}>
                        <message.user.icon className="fs-22 fill-white" />
                      </span>
                    </span>}
                  {message.user.avatar && <span className="flex-shrink-0">
                      <Image src={message.user.avatar} height={36} width={36} className="avatar-md rounded-circle" alt="User Avatar" />
                    </span>}
