...
const sidenavColorOptions = [{
  ...
  color: 'gray',
  image: graySideNavImg
}, {
  color: 'gradient',
  image: gradientSideNavImg
}, {
  color: 'image',
  image: imageSideNavImg
}];
const sidenavSizeOptions = [{
  ...
  size: 'compact',
  image: compactSideNavImg,
  label: 'Compact'
}, {
  size: 'condensed',
  image: smallSideNavImg,
  label: 'Condensed'
}, {
  size: 'on-hover',
  image: smallSideNavImg,
      ...
      backgroundImage: `url(${pattern})`
            ...
            <SimpleBar className="offcanvas-body p-0 h-100">
                <div className="p-3 border-bottom border-dashed">
                    ...
                    <Row className="g-3">
                        {skinOptions.map((item, idx) => <Col sm={6} key={idx}>
                                <div className="form-check card-radio">
                                    <input id={`skin-${item.skin}`} className="form-check-input" type="radio" name="data-skin" disabled={item.disabled ?? false} value={item.skin} checked={skin === item.skin} onChange={() => changeSkin(item.skin)} />
                                    <label className="form-check-label p-0 w-100" htmlFor={`skin-${item.skin}`}>
                                        <Image src={item.image} alt="layout-img" className="img-fluid" width={167.5} height={111.66} />
                                    </label>
                                </div>
                        ...
                        {themeOptions.map((item, idx) => <Col sm={4} key={idx}>
                                <div className="form-check card-radio">
                                    ...
                                    <label className="form-check-label p-0 w-100" htmlFor={`theme-${item.theme}`}>
                                        <Image src={item.image} alt="layout-img" className="img-fluid" width={104.33} height={83.45} />
                                    </label>
                                </div>
                ...
                <div className="p-3 border-bottom border-dashed">
                    ...
                    <Row className="g-3">
                        {orientationOptions.map((item, idx) => <Col sm={4} key={idx}>
                                <div className="form-check card-radio">
                                    <input id={`layout-${item.orientation}`} className="form-check-input" type="radio" name="data-layout" value={item.orientation} checked={orientation === item.orientation} onChange={() => changeOrientation(item.orientation)} />
                            ...
                            <div className="d-flex justify-content-between align-items-center">
                                ...
                                <div className="btn-group radio" role="group">
                                    {layoutPositionOptions.map((item, idx) => <Fragment key={idx}>
                                            <input type="radio" className="btn-check" name="data-layout-position" id={`position-${item.position}`} value={item.position} checked={position === item.position} onChange={() => changeLayoutPosition(item.position)} />
            ...
            <div className="offcanvas-footer border-top p-3 text-center">
                <Row>
                    ...
                    <Col sm={6}>
                        <Link href="https://wrapbootstrap.com/theme/inspinia-multipurpose-admin-dashboard-template-WB0R5L90S?ref=inspinia" target="_blank" className="btn btn-danger bg-gradient py-2 fw-semibold w-100">
                            Buy Now
                        </Link>
                    </Col>
                </Row>
